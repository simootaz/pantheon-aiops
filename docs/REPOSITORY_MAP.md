...
### The repository is tool-neutral

[ADR 0003](adr/0003-neutral-repository-documentation.md) · live now

No tracked file — path or contents — references an assistant tool, and no commit
message does either. This file, `docs/REPOSITORY_MAP.md`, is the canonical map;
a root-level pointer file is permitted but **gitignored**, never tracked.

Model configuration is provider-neutral (`PANTHEON_LLM_PROVIDER`,
`PANTHEON_LLM_BASE_URL`, `PANTHEON_LLM_API_KEY`, `PANTHEON_LLM_MODEL`) for the
same reason object storage is: no welding to one vendor.

> **The rule:** enforced by `tests/unit/test_repo_neutrality.py`, which scans
> every `git ls-files` path and its contents. Commit messages carry no tool
