...
| **Moira** | [agents/capacity/](../agents/capacity/) | Says when a filling resource crosses its limit | 5 |
| **Mnemosyne** | [agents/knowledge/](../agents/knowledge/) | Has this happened before, and what did we conclude | 5 |
...
| `api/routers/webhooks.py` → `POST /webhooks/github` | Verifies GitHub's HMAC-SHA256 over the **raw body**, then parses. Taking a parsed `dict` and re-serialising it produces different bytes - different whitespace, different key order - so that version rejects every genuine delivery; the worse version accepts a body it never verified. **Order matters**: an unsigned malformed body is a 401, not a 400, which is the observable proof the parser never saw it. `GITHUB_WEBHOOK_SECRET` is **required in production**, the same split as GitLab - a token reaches out, a secret guards something reaching in. An event nobody acts on is still 202: a 400 would make a green delivery log go red for working correctly. | 4 | Both hooks now **run Zeus** rather than only publishing, through the one runner in `_runs.py` - it lived private in `alerts.py` while it had one caller, and copying it would have been three implementations of "run Zeus safely" with the drifting one belonging to whichever endpoint nobody exercises. Nothing is scheduled for a trigger the classifier routes nowhere, and `investigating` says which happened: a 202 alone cannot distinguish a `star` from a failed workflow run.
...
| `core/orchestrator/hypotheses.py` | Ranks correlated Findings into candidate causes, and **refuses to name what it cannot**. One signal is not a metric: `FLAKE_SIGNAL` names `FLAKY_TEST` from `PipelineRunPayload.attempt_conclusions` - two recorded outcomes for one job at one commit, the definition applied a second time rather than the word `flake` read off a tag. A ranker that read the tag survived every test through Hephaestus, because Hephaestus writes both; the test that separates them sets the label against the thing. A signal that *names* a cause and one that is a symptom are different: `pantheon_pod_memory_working_set_bytes` **is** resident memory, so growth naming a leak is semantics, not a heuristic. Errors, latency and CPU corroborate and propose nothing - so `bad_deploy_5xx` and `noisy_neighbor` come back **UNKNOWN with their evidence attached**, because nothing reports deployments and nothing knows which pods share a node. Both were committed as predicted misses *before* the module existed: the ground truth is in this repository and a mapping that named all five would have been fitted to it. | 2 |
...
| `core/orchestrator/classifier.py` → `Classification.explains` | Whether the run owes an **explanation** or an **answer**. Every verdict went through `hypotheses.rank`, so a question answered by Hermes came back with an UNKNOWN root cause - "1 finding(s) that nothing here can attribute to a cause" - under an answer never claimed to be about an incident, and a summary saying Hermes "looked and found nothing: no metric crossed its calibrated threshold" beside the Finding that was the whole point. A pull-request review got the same. The classifier had said "answered rather than investigated" in its reason all along; this is that reason as a field the aggregator reads. Defaults to True, so a branch that forgets it diagnoses rather than goes quiet. | 5 |
| `core/orchestrator/classifier.py` → `SCHEDULED_JOBS` | Job name to domains, closed. `delivery-health` → Themis. A scheduled measurement does not explain. | 5 |
| `core/orchestrator/classifier.py` → `ALERT_DOMAINS` | Four domains for an alert: `anomaly`, `log_clustering`, `capacity`, `knowledge`. Moira reads every alert rather than only ones naming a disk, for the reason Argus scans every metric - a trigger's labels are operator-supplied text, and a fill is a fact about the window, not about the alertname. | 5 |
| `core/bus.py` → `subscribe` | In-process fan-out, per investigation, from now on. From now on rather than from the start: a subscriber that wants the past reads the store, and a bus that replayed history would be a second copy of it with weaker guarantees. A raising subscriber is logged and skipped - a client that went away must not fail the run that was publishing - and delivery iterates a copy, because a subscriber that unsubscribes itself mid-delivery would otherwise skip its neighbour. **Correct while Zeus runs in the API process**, which it does; a separate worker is the trigger for the shared bus, and the reason `guarantee` still says AT_MOST_ONCE. | 5 |
| `core/orchestrator/dispatcher.py` | Two registries. `AGENTS` is what a plan may name; `IMPLEMENTATIONS` is every agent whose code exists, and it is the **wider** set. `register(..., dispatchable=False)` records an agent that is written and that nothing routes to yet - Themis, until something schedules anything. One dict made "built" and "reachable" the same answer, and the roster reported an agent that runs as one that does not exist. Neither is discovered by import scanning: a registry that walks the filesystem will one day find a half-written agent, and the failure surfaces at dispatch. | 2 |
| `core/orchestrator/router.py` | Zeus's entrypoint, and where a verdict's recommended remediations **reach a person**. Three finished pieces sat unjoined until this: `open_request` had no production caller, `InvestigationState.AWAITING_APPROVAL` was declared and never set, and `ApprovalRequestedEvent` was rendered by the AG-UI translator and published by nothing. A run with something waiting is **AWAITING_APPROVAL with no `completed_at`** and publishes **no** completion event - that event says a run reached a terminal state, and the translator maps it to RUN_FINISHED, which would close the stream on the reader being asked to answer. A verdict recommending something with no gate **raises**: dropping it would report the run complete with nobody asked. | 2 |
...
| `core/guardrails/proposal.py` | **The request half of the gate.** `propose` evaluates one Action and opens an approval request when policy says a person must decide; `executor.execute` is the spend half. Two functions because they happen at two moments, minutes or hours apart - one call doing both would block on a human, and a retry would open a second request for the same act. A DENY opens nothing: `execute` re-evaluates and refuses anyway, but an audit trail holding an approval for an Action policy denied says a person authorised what the system had already refused. `ready` is a property rather than `approval is None`, which is true for an ALLOW **and** for a DENY. | 4 |
| `core/guardrails/approval_gate.py` | An Action waits here for a person. An approval is for **the content the approver read**, not for an id - a digest of the decision-relevant fields, so proposing a dry run, getting it approved and flipping `dry_run` is refused. The timeout **fails closed**, and an approval decays to EXPIRED while a rejection does not: authority granted at noon should not still be spendable at eight. The proposer cannot approve. The request **retains the Action** it was opened for, because a queue of ids and rules is a queue nobody can answer - `core/ui/approval.py` refuses to emit "approve action 7f3a?" and a list endpoint serving no more would force every UI over it back into that prompt. The digest is **not** served beside it: a caller comparing the two would have verified that a stored copy equals itself. ⚠️ **Nothing opens a request yet** - `open_request` has no production caller, so `execute` refuses an Action needing approval and no path turns that refusal into somebody being asked. | 3 |
...
| `agents/capacity/forecast.py` | Three numbers, two of them definitions. A least-squares slope **is** what "rate" means; `(limit − current) / rate` **is** what "time to limit" means; neither has a parameter. R² is the third and the only one where a forecaster gets to lie, so it **travels with the projection** rather than suppressing it - a poor fit produces a Finding with a poor fit on it. Two points are refused: they always fit perfectly and say nothing. | 5 |
| `agents/capacity/agent.py` | **Moira.** Projects to the metric's **own limit** - a disk is full at used == total - and never to a substitute: the kubelet's eviction threshold is configuration it cannot read, and a default of 85 % would be a number that means something else on every cluster where it was set differently. Memory is **DEGRADED on every run**, including `memory_leak`, because no limit metric exists and the connector that would read one is a Go stub; predicted as a miss before the agent was written. Reads its **own 24-hour window** rather than the five-minute incident window - five minutes of a three-day fill is noise with a slope through it - and puts the window it read on the Finding. A fill past `HORIZON_HOURS` is a clean window, not a risk: every disk fills eventually. Reachable on every alert through `ALERT_DOMAINS`. Predictions and results in `docs/moira-predictions/`; one of six missed, by the generator's seasonal term, and the miss stands in the record. | 5 |
| `core/memory/recall.py` | **The first memory query is exact.** The same `alertname` and the same subject labels the current alert carries, in this tenant - a keyed lookup on the trigger's own labels, not a search for anything "similar" that would need a threshold nobody measured. Scans `recent(200)` and filters in Python, because that is what the store answers; ADR 0008's volume trigger is now the trigger for a query here too. The tenant is fixed by the runtime from the run in progress: the tool has no parameter for another one. | 5 |
| `agents/knowledge/agent.py` | **Mnemosyne.** One Finding when there are priors - count, newest listed, what the last verdict concluded - and none when there are not: "no history" is what the step's clean completion already says. **History is for the person, not the ranker**: `hypotheses.rank` excludes a Finding whose evidence is only prior incidents, because it shares the subject and would otherwise corroborate - last Tuesday's verdict raising confidence in this Tuesday's. `find_runbook` is gone from the manifest: nothing stores a runbook. | 5 |
| `agents/_base/base_agent.py` → `AgentContext.provided` | Tools the **runtime** supplies - ones that close over something only it holds, such as the investigation store. Registered by `run` only when the manifest declares them, so the dispatcher offering `memory.recall` to every agent widens nobody's allowlist. Two guards hold that and hold different halves: `call` refuses an undeclared name whatever is bound; `run` skips rather than registers, because `register` refuses too and a refusal there would degrade every agent on every alert plan. | 5 |
| `core/reporting/timeline.py` | **A rendering, not a Finding.** Every recorded moment of a run - trigger, lifecycle, each step's start and finish, each Finding's report, each consultation, each credential event, the verdict, the close - ordered by when it was written. A Finding is placed **when it was reported**, and its entry says what window it covers: placing it at the window would put an anomaly on the timeline before anyone had noticed it, a timeline that reads as if the system knew things before it did. Ties keep record order, because a fast agent finishes and reports at one second. Clio's manifest no longer claims `write_timeline`: an agent producing this would put "what happened so far" in the Findings list, which is a list of claims about the incident. Served by `GET /investigations/{id}/timeline`, derived on every read. | 5 |
| `agents/dora/delivery.py` | **Review latency is not lead time**, and that distinction is the whole module. DORA lead time runs first-commit-to-production; a pull request carries open-to-merge. Reporting one as the other makes a team look fast by measuring a shorter interval, and it is the most common misuse of these numbers - so the field is `review_latency` and **no performance band is emitted**, because elite/high/medium/low are defined against the metric this does not have. Merge frequency is labelled a **proxy**: it is deployment frequency only if every merge deploys, which is a claim about a pipeline. A median below five merges is refused - it would describe those five, not the team. | 4 |
| `agents/dora/agent.py` | **Themis.** Two of the four DORA metrics are **omitted rather than approximated**: change failure rate and time-to-restore need deployments linked to incidents, and nothing records either. A change-failure rate from "PRs later reverted" measures reverts, and a team that fixes forward would score perfectly while breaking production weekly. `assess_delivery_trend` is gone for the reason Lethe's surge test was deleted - comparing two small windows measures two small samples. Reachable through `/triggers/schedule` and the chart's CronJob - a delivery measurement belongs on a schedule, and one now exists. Its run does not explain: the verdict carries the measurement as the answer, not an UNKNOWN cause under it. | 4 |
...
| `api/routers/agents.py` | The roster and one manifest whole. Each row carries **`implemented` and `dispatchable`**, both read from the dispatcher's registries rather than from the manifest: ten manifests validate, six have code, five can be reached by a trigger. They are two fields because they are two facts - `implemented` used to be read from the *dispatchable* registry, so Themis, which is written and tooled and tested, was reported exactly as Clio, which is a manifest and nothing else. The field's name had stopped naming what it measured. | 1 |
| `api/routers/schedules.py` | The receiver for `TriggerKind.SCHEDULE`, which existed and nothing produced. A named job the classifier maps to a domain, and the subject the agent needs - a trigger receiver in the same sense as `/webhooks/alertmanager`, not the second way in `investigations.py` refuses. An **unknown job is a 422**, unlike GitHub's accept-and-ignore: a schedule is configured by us, and a typo in a CronJob should come back in the response rather than as a run that never appears. Bearer auth, so the run carries the caller's **tenant** - the one receiver with an identity passes it on. It does not need Temporal: `dispatcher.py` names what would, and firing a request at 06:00 is not on the list. | 5 |
...
| `api/agui/translator.py` | **The state is patched**: `running` on start, `awaiting_approval` on an approval request, the terminal state and `completed_at` on completion - before `RUN_FINISHED`, so a client applies the end before the stream closes on it. Nothing else on the stream carried the state, and the opening snapshot was the only place it was ever written; a client that opened during PENDING showed `pending` forever. Internal events become AG-UI events **here and nowhere else** - `core/contracts/events.py` stays the bus envelope, and conflating the two would make every internal change a protocol change. The shared state object is the **Investigation**: a snapshot at the start, RFC 6902 patches after. Every list patch is an **append**, never a `replace` at an index - a replace applies cleanly and lands on the wrong element the moment two agents finish out of order. An unmapped event **raises**: silence at the edge is indistinguishable from a run where nothing happened. | 4 |
| `api/agui/a2ui_channel.py` | The **one seam** where an A2UI payload enters the stream, because no canonical AG-UI envelope for one is specified in either spec. The guess is bounded to `EVENT_NAME` and `to_wire`. Surfaces are built here from `Action` and `AccessRequest`, so the component allowlist is a statement about what this module emits rather than a filter on hostile input. An approval card carries the target, blast radius and rollback: "approve action 7f3a?" is a prompt people learn to click through. | 4 |
| `api/agui/endpoint.py` | SSE for **one** investigation, 404 for another tenant's - the same reasoning as the investigation reads. **It was never live in the running app**: it read `app.state.agui_subscribe` and only a test ever set it, so the stream sent a snapshot and closed, and the dashboard reconnected every one to thirty seconds for as long as a page stayed open. `api/main.py` now bridges the bus to it. The **keepalive existed in prose**: the timeout branch was a bare `continue` that sent the proxy nothing; it yields an SSE comment now, and the test for it is bounded because a plant of the old branch hung the suite rather than failing it. A client whose `X-A2UI-Components` lacks something an approval prompt is built from is refused with a **406** before streaming, **after** the tenant check so a 406 cannot confirm another tenant's run exists; no header streams and is flagged `X-A2UI-Capabilities: undeclared`. The docstring claimed this handshake long before anything did it. The stream **closes on a terminal event**; one left open is a file descriptor per abandoned tab. `POST /actions` excludes VIEWER deliberately: every action it accepts is a decision, and it parses rather than executes, because `may_execute` re-validates the approval against the Action *as it is now*. | 4 |
| **core/ui/** | A2UI surface construction, restricted to the `A2UIComponentType` allowlist. | 4 |
| `dashboard/lib/session.ts` | Where the token comes from, and **what is honestly missing**: there is no sign-in flow, no login endpoint and no IdP, so the token is read from `sessionStorage` under one documented key and a view with no token *says so*. It does not render an empty list - "you are not signed in" and "there are no investigations" are different facts, and showing the second for the first sends somebody looking for a run that is sitting right there. `sessionStorage` over `localStorage`: it dies with the tab, so a shared machine does not carry a credential into the next person's session. | 4 |
| `dashboard/lib/api.ts` | The REST reads, in one place so the token is attached once - **in a header**, the same rule as `stream.ts` and both connectors. A refusal **throws `ApiError` carrying the status**; a 401 turned into `[]` renders "no investigations" to somebody whose token expired. `recentInvestigations` passes **no tenant** and must not gain the ability to: a `?tenant=` would be a claim rather than a fact, and the endpoint would become an invitation to read another tenant's runs by typing their name. | 4 |
| `dashboard/lib/investigations.ts` | Reading a run: `isPartial` is **derived from a DEGRADED finding being present**, never from a flag. `InvestigationCompletedEvent` carries `partial`, but the Investigation does not and must not - partiality is not a claim the orchestrator makes, it *is* an agent having said on the record that it could not do its job. A verdict shown beside a findings count, with no note that the log search never ran, is a conclusion drawn from evidence half of which was never collected. | 4 |
| `dashboard/components/states.tsx` | Loading, signed-out, refused and empty as **four separate components**, because they are four different facts and each one rendered for another sends a reader to the wrong place. Components rather than a convention, so a new view gets all four by using them instead of by remembering that four exist. | 4 |
| `dashboard/components/investigations.tsx` | The row, the status and the gap list. `Status` reads **both the connection and the run state**: a finished run and a dropped stream both stop producing events, and on `connected` alone every completed investigation sits there claiming to reconnect forever. They live here rather than in the pages because a Next page module may export nothing but its route contract - which conveniently makes them renderable by a test with no route, session or stream. | 4 |
| `dashboard/lib/agui/stream.ts`, `use-investigation.ts` | Reads the AG-UI stream with **`fetch`, not `EventSource`** - EventSource cannot set headers, so authenticating one means a token in the query string, which lands in the proxy's access log. The connectors refuse exactly that on the way out; it would be strange to argue it there and put the dashboard's token in a URL on the way in. A **reconnect discards the old state**: patches from a fresh stream applied to state from an old one produce a run that never happened. A 401, 403 or 404 is not retried - retrying produces a log full of failures and never a connection. | 4 |
| `core/ui/components.py` | One builder per allowlisted component, so the **signature** says which fields that type takes - a Divider builder has no `text` parameter, so a Divider carrying text is not expressible by accident. `image` takes an ArtifactRef and **has no URL parameter**. A Button's action is required: one that does nothing when pressed is indistinguishable from one whose answer was lost. | 4 |
| `core/ui/approval.py`, `core/ui/access_request.py` | The surfaces, built from `Action` and `AccessRequest` and never from agent text - which is what makes the allowlist a statement about this module rather than a filter on hostile input. The approval card carries target, blast radius, rollback and reason: **"approve action 7f3a?" is a prompt people learn to click through**, and a gate answered by reflex records a decision nobody made. | 4 |
| `core/ui/artifact_resolution.py` | ⛔ **Resolves an ArtifactRef to a signed URL.** Server-side only; agents must not import it - mirroring `cerberus/redemption.py`, where the agent holds a reference and the server holds the capability. The **bucket is fixed from configuration**, never from the reference, which is what makes "no arbitrary destination is expressible" true rather than descriptive. Signing is **injected**: a hand-rolled SigV4 would be unverifiable security-critical code with no S3 here to check it against. | 4 |
| `dashboard/components/approvals.tsx` | The approval card in the browser, carrying what `core/ui/approval.py` puts on the A2UI surface - target, blast radius, rollback, dry-run, and why. Not an id, for the reason that module gives. A **rejection must state a reason**: the API accepts an empty one, this does not offer it, because a proposer told only "no" learns nothing about what to change. A request whose Action is missing is **listed but not answerable** - hiding it would lose a request genuinely waiting, and a button beside it would be the click-through prompt. | 4 |
| `dashboard/app/approvals/page.tsx` | The queue. **Refetches after an answer instead of removing the row**: the gate can refuse one - already answered, expired, a proposer answering their own - and a row vanishing optimistically would read as success. A refusal lands on its own card, because the other rows are still answerable. | 4 |
| `dashboard/components/agents.tsx` | Three standings, because there are three facts: **Live** (implemented and dispatchable), **Not routed** (written, and nothing produces a plan that names it), **Declared** (a manifest and nothing else). Each is spelled out in words, not signalled by a colour - a reader who does not already know the difference learns it from the row. | 4 |
| `dashboard/components/providers.tsx` | Provider cards, tier pickers, probe. The model list **says where it came from**: a live answer and a `manual_models` fallback look identical on screen and mean opposite things, and binding a tier against the second is the 03:00 failure ADR 0004 exists to move to settings time. The probe is a **button, never an effect** - every probe is a paid request charged to whoever pressed it - and it probes only the bound models, because a provider can list dozens. `auth_mode: none` with no key is **not** reported as a fault: that is a local provider that wants no credential, and a warning there teaches people to ignore warnings. No key field, masked or otherwise. | 4 |
| `dashboard/app/settings/page.tsx` | Provider settings only. ⚠️ **Per-agent model overrides are absent on purpose**: `ResolutionPolicy.per_agent` is read by the resolver and nothing persists or serves one - no store, no endpoint - so a picker would write to nowhere and read back its own optimism. Cerberus grants likewise: the grant book is in-process and has no API. A settings control that changes nothing is worse than an absent one, because somebody sets it and believes it took. | 4 |
| **dashboard/** | TypeScript. Next.js 15 App Router — the only TS in the repo. | 4 |
| `dashboard/app/` | Routes: `investigations/` (list and `[id]` detail), `agents/`, `approvals/`, `settings/`. `agents/` is the one view with **no sign-in guard**: `GET /agents` takes no principal, so there is nothing to narrow, and a prompt in front of it would claim there was. | 4 |
| `dashboard/lib/agui/` | The stream reader (`stream.ts`, `fetch` - not `HttpAgent`, not `EventSource`), the hook (`use-investigation.ts`) and the Investigation state store: `StateSnapshot` then RFC 6902 `StateDelta`. The stream **declares the renderer's catalog** in `X-A2UI-Components`, generated from `ALLOWED_COMPONENTS`, and a **406** is not retried because the renderer does not change between attempts. `client.ts` was deleted: it POSTed to `/agui`, a route that never existed, and nothing imported it - see the amendment to ADR 0006. | 4 |
...
| `dashboard/lib/` | API client, session, and the pure reads over an Investigation. | 4 |
...
| `simulator/pipeline_generator.py` | Forge webhooks posted to the **real** endpoint over real HTTP - no simulator-only route. `GitHubGenerator` sends `workflow_run` and `pull_request` deliveries **signed over the exact bytes posted**, with the secret the receiver reads, which is the protocol and not a bypass. The runner used to send GitLab pipeline hooks, and every one was a 202 that **investigated nothing**: the classifier reads GitHub's shapes, and no gate noticed because every integration test runs with `send_pipelines=False`. It delivers the trigger, not the world behind it - Hephaestus then asks the real GitHub API about `acme/checkout` and degrades, correctly. GitLab builders kept. | 1 |
...
| `deploy/helm/pantheon/` | Helm chart: `Chart.yaml`, three values files, `templates/`. **`PANTHEON_ENV` is a value, not the namespace**: it was `{{ .Release.Namespace }}` for two phases, and `Environment` is a closed enum, so any install in a namespace not literally named `local`/`ci`/`staging`/`production` - or `helm template`'s own `default` - crashed the API at startup on a validation error no rendered manifest could show. `api.existingSecret` holds the credentials the app refuses to start without in production; the render **fails closed** without it, and `tests/unit/test_repo_structure.py` holds the documented key list equal to `REQUIRED_IN_PRODUCTION`. `postgres:` and `redis:` are values with **no templates behind them** yet. | 6 |
| `deploy/helm/pantheon/templates/schedules-cronjob.yaml` | One CronJob per configured job, POSTing to `/triggers/schedule` with an OPERATOR token from a secret. **Off by default and fails closed**: the chart does not mint API tokens, and a CronJob posting with none would produce a 401 every Monday that nobody reads. `curl --fail`, so a 4xx is a failed Job somebody can see. | 5 |
